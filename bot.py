...
def apply_template(
    ...
    # ── 1. Abre e mantém proporção original ──────────────────────────────────
    img = Image.open(io.BytesIO(photo_bytes)).convert("RGBA")
    w, h = img.size
    # Redimensiona mantendo proporção se a imagem for maior que 1080px
    ...
    grad_height = int(h * GRADIENT_RATIO)
    ...
    banner_w = int(w * BANNER_WIDTH_PCT)
    ...
    bx = (w - banner_w) // 2
    by = h - banner_h - int(h * BANNER_MARGIN)
